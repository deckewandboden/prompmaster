...
test('non-WebGL fallback keeps the actual head visible without camera access',()=>{
  ...
  assert.match(fallback,/context\.globalAlpha=clamp\(alphaBase\+pulse\*alphaPulse,0,1\)/);
  ...
  assert.match(fallback,/point\.color\.map\(value=>Math\.round\(clamp\(value\*1\.72,0,1\)\*255\)\)/);
  assert.match(fallback,/rgba\(\$\{r\},\$\{g\},\$\{b\},\.82\)/);
  ...
  assert.match(fallback,/drawSceneLayers\(sceneLayers\.landscape,\.72,\.075,\.018,\.22,\.006,\.292683,\.707317\)/);
  assert.match(fallback,/drawSceneLayers\(sceneLayers\.blend,\.68,\.075,0,\.24,\.008,\.275862,\.724138\)/);
