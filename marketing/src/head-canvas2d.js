...
export async function initCanvasHead({sourceCanvas,stage,fallback}){
  ...
  function draw(now){
    ...
    for(let i=0;i<surface.seeds.length;i++){
      ...
      addPointToBatch(surfaceBatches,sx2,sy2,renderSize,intensity,clamp(alpha*.60,0,1));
    }
    ...
    for(let i=0;i<topology.detail.length;i++){
      ...
      addPointToBatch(topologyBatches,sx2,sy2,renderSize,intensity,clamp(alpha*.55,0,1));
    }
