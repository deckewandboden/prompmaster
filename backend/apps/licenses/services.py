...
def assign_license(license, user, actor):
    # ``company`` is nullable (private licenses). PostgreSQL rejects FOR UPDATE
    # when it implicitly locks the nullable side of the LEFT JOIN generated by
    # select_related(). Lock only the License row; related rows are read-only
    # here and tenant membership/user state is locked independently below.
    lic = (
...
def consume_assignment_link(*, raw_token, user, request=None):
    ...
    link = (
        LicenseAssignmentLink.objects.select_for_update(of=('self',))
