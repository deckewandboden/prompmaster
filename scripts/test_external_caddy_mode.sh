...
grep -q 'promptmaster_v2.20260922.css?v=20260924-dbfree2' <<<"$free_current" || {
  echo "Free V2 stylesheet is not cache-busted through external Caddy" >&2
  exit 1
}
grep -q 'promptmaster_ui_v2.20260922.js?v=20260924-dbfree2' <<<"$free_current" || {
...
mutable_headers="$(
  docker run --rm --network "$NETWORK" curlimages/curl:8.12.1 \
    ...
    "http://$ALIAS/static/css/promptmaster_v2.20260922.css?v=20260924-dbfree2"
)"
